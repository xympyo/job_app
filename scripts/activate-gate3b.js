...
async function main() {
  ...
  if (existing.length === 1 && existing[0].current_revision_id) {
    ...
    const report = { generatedAt: new Date().toISOString(), mode: "verification-only", ownerEmail, candidateHash: candidate.canonicalHash, backup: { path: backupPath, sha256: backupHash, generatedAt: backup.generatedAt }, before, after, schema, rls: { ownerVisible: 1, otherAuthenticatedVisible: 0, anonymousAccess: "denied", publishedRevisionMutation: "rejected by immutability trigger" }, activation: { profileId: existing[0].id, revisionId: existing[0].current_revision_id, revisionNumber: 1, status: "published" }, semanticMatch: true, note: "Already activated; no duplicate profile or revision was created." };
